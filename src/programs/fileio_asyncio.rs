...
unsafe extern "C" fn AIO_fwriteSparse(
    ...
    while ptrT < bufferTEnd {
        ...
        while nb0T < seg0SizeT
            && *ptrT.offset(nb0T as isize) == 0 as std::ffi::c_int as size_t
        {
        ...
        if nb0T != seg0SizeT {
            ...
            if fseek(file, storedSkips as std::ffi::c_long, SEEK_CUR)
                != 0 as std::ffi::c_int
            {
                if g_display_prefs.displayLevel >= 1 as std::ffi::c_int {
                    fprintf(stderr, b"zstd: \0" as *const u8 as *const std::ffi::c_char);
    ...
    if bufferSize & maskT != 0 {
        ...
        storedSkips = storedSkips
            .wrapping_add(
                restPtr.offset_from(restStart) as std::ffi::c_long as std::ffi::c_uint,
            );
        if restPtr != restEnd {
...
unsafe extern "C" fn AIO_fwriteSparseEnd(
    ...
    if (*prefs).testMode != 0 {
        if storedSkips == 0 as std::ffi::c_int as std::ffi::c_uint {} else {
            __assert_fail(
    ...
    if storedSkips > 0 as std::ffi::c_int as std::ffi::c_uint {
        if (*prefs).sparseFileSupport > 0 as std::ffi::c_int {} else {
            __assert_fail(
        ...
        if fseek(
            file,
            storedSkips.wrapping_sub(1 as std::ffi::c_int as std::ffi::c_uint)
                as std::ffi::c_long,
            SEEK_CUR,
        ) != 0 as std::ffi::c_int
        {
        ...
        let lastZeroByte: [std::ffi::c_char; 1] = [
...
unsafe extern "C" fn AIO_IOPool_createIoJob(
    ...
    let job = malloc(::core::mem::size_of::<IOJob_t>() as std::ffi::c_ulong)
        as *mut IOJob_t;
...
unsafe extern "C" fn AIO_IOPool_createThreadPool(
    ...
    if (*prefs).asyncIO != 0 {
        if pthread_mutex_init(&mut (*ctx).ioJobsMutex, 0 as *const pthread_mutexattr_t)
            != 0
        {
            if g_display_prefs.displayLevel >= 1 as std::ffi::c_int {
                fprintf(stderr, b"zstd: \0" as *const u8 as *const std::ffi::c_char);
        ...
        if 10 as std::ffi::c_int >= 2 as std::ffi::c_int {} else {
            __assert_fail(
...
unsafe extern "C" fn AIO_IOPool_init(
    ...
    while i < (*ctx).availableJobsCount {
        (*ctx)
            .availableJobs[i
            as usize] = AIO_IOPool_createIoJob(ctx, bufferSize) as *mut std::ffi::c_void;
        i += 1;
        i;
    }
...
unsafe extern "C" fn AIO_IOPool_releaseIoJob(mut job: *mut IOJob_t) {
    ...
    (*ctx).availableJobs[fresh0 as usize] = job as *mut std::ffi::c_void;
...
unsafe extern "C" fn AIO_IOPool_setThreaded(
    ...
    if threaded == 0 as std::ffi::c_int || threaded == 1 as std::ffi::c_int {} else {
        __assert_fail(
    ...
    'c_16016: {
        if threaded == 0 as std::ffi::c_int || threaded == 1 as std::ffi::c_int {} else {
            __assert_fail(
                b"threaded == 0 || threaded == 1\0" as *const u8
                    as *const std::ffi::c_char,
                b"/home/peter/Dev/zstd-c2rust/programs/fileio_asyncio.c\0" as *const u8
                    as *const std::ffi::c_char,
                240 as std::ffi::c_int as std::ffi::c_uint,
...
unsafe extern "C" fn AIO_IOPool_acquireJob(mut ctx: *mut IOPoolCtx_t) -> *mut IOJob_t {
    ...
    AIO_IOPool_lockJobsMutex(ctx);
    if (*ctx).availableJobsCount > 0 as std::ffi::c_int {} else {
        __assert_fail(
...
unsafe extern "C" fn AIO_IOPool_enqueueJob(mut job: *mut IOJob_t) {
    let ctx = (*job).ctx as *mut IOPoolCtx_t;
    if AIO_IOPool_threadPoolActive(ctx) != 0 {
        POOL_add((*ctx).threadPool, (*ctx).poolFunction, job as *mut std::ffi::c_void);
    } else {
        ((*ctx).poolFunction)
            .expect("non-null function pointer")(job as *mut std::ffi::c_void);
    };
}
#[no_mangle]
...
pub unsafe extern "C" fn AIO_WritePool_setFile(
    ...
    if (*ctx).storedSkips == 0 as std::ffi::c_int as std::ffi::c_uint {} else {
        __assert_fail(
...
unsafe extern "C" fn AIO_ReadPool_addJobToCompleted(mut job: *mut IOJob_t) {
    ...
    if (*ctx).completedJobsCount < 10 as std::ffi::c_int {} else {
        __assert_fail(
    ...
    (*ctx).completedJobs[fresh1 as usize] = job as *mut std::ffi::c_void;
...
unsafe extern "C" fn AIO_ReadPool_getNextCompletedJob(
    ...
    while job.is_null()
        && AIO_ReadPool_numReadsInFlight(ctx) > 0 as std::ffi::c_int as size_t
    {
        if !((*ctx).base.threadPool).is_null() {} else {
            __assert_fail(
                b"ctx->base.threadPool != NULL\0" as *const u8
                    as *const std::ffi::c_char,
                b"/home/peter/Dev/zstd-c2rust/programs/fileio_asyncio.c\0" as *const u8
                    as *const std::ffi::c_char,
    ...
    if !job.is_null() {
        ...
        (*ctx)
            .waitingOnOffset = ((*ctx).waitingOnOffset as std::ffi::c_ulong)
...
unsafe extern "C" fn AIO_ReadPool_executeReadJob(mut opaque: *mut std::ffi::c_void) {
    ...
    if (*job).usedBufferSize < (*job).bufferSize {
        if ferror((*job).file) != 0 {
            ...
            if g_display_prefs.displayLevel >= 1 as std::ffi::c_int {
                fprintf(stderr, b"Read error\0" as *const u8 as *const std::ffi::c_char);
            }
            if g_display_prefs.displayLevel >= 1 as std::ffi::c_int {
                fprintf(stderr, b" \n\0" as *const u8 as *const std::ffi::c_char);
            }
...
unsafe extern "C" fn AIO_ReadPool_enqueueRead(mut ctx: *mut ReadPoolCtx_t) {
    ...
    (*ctx)
        .nextReadOffset = ((*ctx).nextReadOffset as std::ffi::c_ulong)
...
pub unsafe extern "C" fn AIO_ReadPool_create(
    ...
    let ctx = malloc(::core::mem::size_of::<ReadPoolCtx_t>() as std::ffi::c_ulong)
    ...
    (*ctx)
        .coalesceBuffer = malloc(bufferSize * 2 as std::ffi::c_int as size_t) as *mut U8;
...
unsafe extern "C" fn AIO_ReadPool_releaseCurrentHeldAndGetNext(
    ...
    (*ctx)
        .currentJobHeld = AIO_ReadPool_getNextCompletedJob(ctx) as *mut std::ffi::c_void;
...
pub unsafe extern "C" fn AIO_ReadPool_fillBuffer(
    ...
    if useCoalesce != 0 {
        ...
        (*ctx)
            .srcBufferLoaded = ((*ctx).srcBufferLoaded)
            .wrapping_add((*job).usedBufferSize);
    } else {
...
unsafe extern "C" fn run_static_initializers() {
    segmentSizeT = ((32 as std::ffi::c_int
        * ((1 as std::ffi::c_int) << 10 as std::ffi::c_int)) as std::ffi::c_ulong)
        .wrapping_div(::core::mem::size_of::<size_t>() as std::ffi::c_ulong);
