...
pub unsafe extern "C" fn BMK_isSuccessful_runOutcome(
    ...
    return (outcome.error_tag_never_ever_use_directly == 0 as std::ffi::c_int)
        as std::ffi::c_int;
}
#[no_mangle]
...
pub unsafe extern "C" fn BMK_benchFunction(
    ...
    nbLoops = nbLoops
        .wrapping_add((nbLoops == 0) as std::ffi::c_int as std::ffi::c_uint);
    let mut i: size_t = 0;
...
pub unsafe extern "C" fn BMK_resetTimedFnState(
    ...
    (*timedFnState)
        .timeBudget_ns = (total_ms as PTime as std::ffi::c_ulonglong)
        .wrapping_mul(TIMELOOP_NANOSEC)
        .wrapping_div(1000 as std::ffi::c_int as std::ffi::c_ulonglong) as PTime;
    (*timedFnState)
        .runBudget_ns = (run_ms as PTime as std::ffi::c_ulonglong)
        .wrapping_mul(TIMELOOP_NANOSEC)
        .wrapping_div(1000 as std::ffi::c_int as std::ffi::c_ulonglong) as PTime;
    (*timedFnState)
        .fastestRun
        .nanoSecPerRun = TIMELOOP_NANOSEC as std::ffi::c_double
...
pub unsafe extern "C" fn BMK_benchTimedFn(
    ...
    while completed == 0 {
        ...
        let loopDuration_ns = newRunTime.nanoSecPerRun
            * (*cont).nbLoops as std::ffi::c_double;
        (*cont)
            .timeSpent_ns = ((*cont).timeSpent_ns as std::ffi::c_ulonglong)
            .wrapping_add(loopDuration_ns as std::ffi::c_ulonglong) as PTime as PTime;
        if loopDuration_ns
            > runBudget_ns as std::ffi::c_double
                / 50 as std::ffi::c_int as std::ffi::c_double
        {
            ...
            (*cont)
                .nbLoops = ((runBudget_ns as std::ffi::c_double / fastestRun_ns)
                as std::ffi::c_uint)
        ...
        if loopDuration_ns < runTimeMin_ns as std::ffi::c_double {
            if completed == 0 as std::ffi::c_int {} else {
                __assert_fail(
