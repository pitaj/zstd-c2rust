...
pub const SAMPLESIZE_MAX: std::ffi::c_int = 128 as std::ffi::c_int
    ...
    while nbSamplesLoaded < sstSize && fileIndex < nbFiles {
        ...
        if fileSize <= 0 as std::ffi::c_int as S64 {
            ...
            if f.is_null() {
                ...
                fprintf(
                    stderr,
                    b"zstd: dictBuilder: %s %s \0" as *const u8
            ...
            fileDataLoaded = if targetChunkSize > 0 as std::ffi::c_int as size_t {
                ...
                (if fileSize
                    < (128 as std::ffi::c_int
                        * ((1 as std::ffi::c_int) << 10 as std::ffi::c_int)) as S64
                {
                    fileSize
                } else {
                    (128 as std::ffi::c_int
            ...
            *sampleSizes.offset(fresh0 as isize) = fileDataLoaded;
            ...
            if targetChunkSize > 0 as std::ffi::c_int as size_t {
                while (fileDataLoaded as S64) < fileSize && nbSamplesLoaded < sstSize {
                    let chunkSize = if (fileSize as size_t).wrapping_sub(fileDataLoaded)
                        < targetChunkSize
                    {
...
unsafe extern "C" fn DiB_findMaxMem(mut requiredMem: std::ffi::c_ulonglong) -> size_t {
    let step = (8 as std::ffi::c_int * ((1 as std::ffi::c_int) << 20 as std::ffi::c_int))
        as size_t;
...
unsafe extern "C" fn DiB_fillNoise(
    mut buffer: *mut std::ffi::c_void,
    mut length: size_t,
) {
...
unsafe extern "C" fn DiB_saveDict(
    ...
    let f = fopen(dictFileName, b"wb\0" as *const u8 as *const std::ffi::c_char);
    if f.is_null() {
        fprintf(
            stderr,
            b"Error %i : \0" as *const u8 as *const std::ffi::c_char,
        ...
        if fileSize == 0 as std::ffi::c_int as S64 {
            ...
            fs.nbSamples
                += ((fileSize as size_t)
                    ...
                    .wrapping_sub(1 as std::ffi::c_int as size_t) / chunkSize)
                    as std::ffi::c_int;
            fs.totalSizeToLoad += fileSize;
        } else {
            if fileSize > SAMPLESIZE_MAX as S64 {
                fs.oneSampleTooLarge
                    |= (fileSize > (2 as std::ffi::c_int * SAMPLESIZE_MAX) as S64)
                        as std::ffi::c_int;
                if displayLevel >= 3 as std::ffi::c_int {
                    fprintf(
                        stderr,
...
pub unsafe extern "C" fn DiB_trainFromFiles(
    ...
    let mut srcBuffer = 0 as *mut std::ffi::c_void;
    ...
    let maxMem = DiB_findMaxMem(
        (fs.totalSizeToLoad * memMult as S64) as std::ffi::c_ulonglong,
    ) / memMult as size_t;
        ...
        || dictBuffer.is_null()
    {
        ...
        fprintf(
            stderr,
            b"not enough memory for DiB_trainFiles\0" as *const u8
                as *const std::ffi::c_char,
        );
        fprintf(stderr, b"\n\0" as *const u8 as *const std::ffi::c_char);
    ...
    if (loadedSize as S64) < fs.totalSizeToLoad {
        if displayLevel >= 1 as std::ffi::c_int {
            fprintf(
                ...
                (fs.totalSizeToLoad
                    / (1 as std::ffi::c_int
                        * ((1 as std::ffi::c_int) << 20 as std::ffi::c_int)) as S64)
                    as std::ffi::c_uint,
                (loadedSize
                    / (1 as std::ffi::c_int
        ...
        if optimize != 0 {
            ...
            if ZDICT_isError(dictSize) == 0 {
                let mut splitPercentage = ((*coverParams).splitPoint
                    * 100 as std::ffi::c_int as std::ffi::c_double) as std::ffi::c_uint;
                if displayLevel >= 2 as std::ffi::c_int {
                    fprintf(
                        stderr,
                        b"k=%u\nd=%u\nsteps=%u\nsplit=%u\n\0" as *const u8
...
unsafe extern "C" fn run_static_initializers() {
        ...
        == 4 as std::ffi::c_int as std::ffi::c_ulong
    {
        ...
        ((512 as std::ffi::c_int * ((1 as std::ffi::c_int) << 20 as std::ffi::c_int))
            as size_t) << ::core::mem::size_of::<size_t>() as std::ffi::c_ulong
    };
