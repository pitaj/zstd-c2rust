...
static mut g_distrib: [std::ffi::c_int; 650] = [
    0 as std::ffi::c_int,
    0,
    0,
    0,
...
unsafe extern "C" fn countFreqs(
    ...
    if g_distribCount <= 650 as std::ffi::c_int as std::ffi::c_uint {} else {
        __assert_fail(
            b"g_distribCount <= DISTRIB_SIZE_MAX\0" as *const u8
                as *const std::ffi::c_char,
            b"/home/peter/Dev/zstd-c2rust/programs/lorem.c\0" as *const u8
                as *const std::ffi::c_char,
...
unsafe extern "C" fn init_word_distrib(
    ...
    while w < nbWords {
        ...
        while l < lmax {
            let fresh0 = d;
            d = d.wrapping_add(1);
            g_distrib[fresh0 as usize] = w as std::ffi::c_int;
            l += 1;
            l;
        }
        w = w.wrapping_add(1);
...
unsafe extern "C" fn LOREM_rand(mut range: std::ffi::c_uint) -> std::ffi::c_uint {
    ...
    rand32 ^= prime2;
    rand32 = rand32 << 13 as std::ffi::c_int
        | rand32 >> 32 as std::ffi::c_int - 13 as std::ffi::c_int;
    g_randRoot = rand32;
    return ((rand32 as std::ffi::c_ulonglong)
        .wrapping_mul(range as std::ffi::c_ulonglong) >> 32 as std::ffi::c_int)
        as std::ffi::c_uint;
}
unsafe extern "C" fn writeLastCharacters() {
    let mut lastChars = g_maxChars.wrapping_sub(g_nbChars);
    if g_maxChars >= g_nbChars {} else {
        __assert_fail(
    ...
    *g_ptr.offset(fresh1 as isize) = '.' as i32 as std::ffi::c_char;
    ...
    if lastChars > 1 as std::ffi::c_int as size_t {
        *g_ptr
            .offset(
                g_maxChars.wrapping_sub(1 as std::ffi::c_int as size_t) as isize,
...
unsafe extern "C" fn generateWord(
    ...
    if upCase != 0 {
        static mut toUp: std::ffi::c_char = ('A' as i32 - 'a' as i32)
            as std::ffi::c_char;
        *g_ptr
            .offset(
                g_nbChars as isize,
            ) = (*g_ptr.offset(g_nbChars as isize) as std::ffi::c_int
            + toUp as std::ffi::c_int) as std::ffi::c_char;
    }
    g_nbChars = (g_nbChars as std::ffi::c_ulong).wrapping_add(strlen(word)) as size_t
        as size_t;
    memcpy(
        g_ptr.offset(g_nbChars as isize) as *mut std::ffi::c_void,
    ...
    g_nbChars = (g_nbChars as std::ffi::c_ulong).wrapping_add(strlen(separator))
        as size_t as size_t;
}
...
unsafe extern "C" fn generateSentence(mut nbWords: std::ffi::c_int) {
    ...
    while i < nbWords {
        ...
        let word = kWords[wordID as usize];
...
unsafe extern "C" fn generateFirstSentence() {
    ...
    while i < 18 as std::ffi::c_int {
        let mut word = kWords[i as usize];
        ...
        generateWord(word, separator, (i == 0 as std::ffi::c_int) as std::ffi::c_int);
        i += 1;
        i;
    }
    generateWord(
        kWords[18 as std::ffi::c_int as usize],
        b". \0" as *const u8 as *const std::ffi::c_char,
        0 as std::ffi::c_int,
    );
}
...
pub unsafe extern "C" fn LOREM_genBlock(
    ...
    if size < 2147483647 as std::ffi::c_int as size_t {} else {
        __assert_fail(
...
pub unsafe extern "C" fn LOREM_genBuffer(
    ...
    LOREM_genBlock(buffer, size, seed, 1 as std::ffi::c_int, 1 as std::ffi::c_int);
}
unsafe extern "C" fn run_static_initializers() {
    kNbWords = (::core::mem::size_of::<[*const std::ffi::c_char; 255]>()
        as std::ffi::c_ulong)
        .wrapping_div(
