...
unsafe extern "C" fn RDG_genBlock(
    ...
    let matchProba32 = (32768 as std::ffi::c_int as std::ffi::c_double * matchProba)
        as U32;
    ...
    while matchProba >= 1.0f64 {
        ...
        size0 = size0
            .wrapping_add(
                RDG_rand(seedPtr) as size_t
                    & size0.wrapping_sub(1 as std::ffi::c_int as size_t),
            );
    ...
    while pos < buffSize {
        if RDG_rand15Bits(seedPtr) < matchProba32 {
            let length = (RDG_randLength(seedPtr))
                .wrapping_add(4 as std::ffi::c_int as U32);
            ...
            let repeatOffset = (RDG_rand(seedPtr) & 15 as std::ffi::c_int as U32
                == 2 as std::ffi::c_int as U32) as std::ffi::c_int as U32;
            let randOffset = (RDG_rand15Bits(seedPtr))
                .wrapping_add(1 as std::ffi::c_int as U32);
            let offset = if repeatOffset != 0 {
                prevOffset
            } else {
                (if (randOffset as size_t) < pos { randOffset as size_t } else { pos })
                    as U32
            };
...
pub unsafe extern "C" fn RDG_genBuffer(
    ...
    RDG_fillLiteralDistrib(
        ldt.as_mut_ptr(),
        (litProba * 256 as std::ffi::c_int as std::ffi::c_double + 0.001f64)
            as fixedPoint_24_8,
    );
    RDG_genBlock(
        buffer,
...
pub unsafe extern "C" fn RDG_genStdout(
    ...
    let mut seed32 = seed;
    let stdBlockSize = (128 as std::ffi::c_int
        * ((1 as std::ffi::c_int) << 10 as std::ffi::c_int)) as size_t;
    let stdDictSize = (32 as std::ffi::c_int
        * ((1 as std::ffi::c_int) << 10 as std::ffi::c_int)) as size_t;
