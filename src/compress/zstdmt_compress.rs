...
unsafe extern "C" fn ZSTDMT_createBufferPool(
    ...
    if pthread_mutex_init(&mut (*bufPool).poolMutex, 0 as *const pthread_mutexattr_t)
        != 0
    {
...
unsafe extern "C" fn ZSTDMT_getBuffer(mut bufPool: *mut ZSTDMT_bufferPool) -> Buffer {
    ...
    buffer.start = start;
    buffer
        .capacity = if start.is_null() { 0 as std::ffi::c_int as size_t } else { bSize };
    start.is_null();
    return buffer;
}
...
unsafe extern "C" fn bufferToSeq(mut buffer: Buffer) -> RawSeqStore_t {
    ...
    seq
        ...
        .wrapping_div(::core::mem::size_of::<rawSeq>() as std::ffi::c_ulong);
    return seq;
}
...
unsafe extern "C" fn ZSTDMT_createCCtxPool(
    ...
    if pthread_mutex_init(&mut (*cctxPool).poolMutex, 0 as *const pthread_mutexattr_t)
        != 0
    {
...
unsafe extern "C" fn ZSTDMT_serialState_init(
    ...
    initError
        |= pthread_cond_init(&mut (*serialState).cond, 0 as *const pthread_condattr_t);
    initError
        |= pthread_mutex_init(
        ...
        if (dstBuff.start).is_null() {
            ...
            if (dstBuff.start).is_null() {
                ...
                (*job)
                    ...
                    as size_t;
                pthread_mutex_unlock(&mut (*job).job_mutex);
                current_block = 16775483362577223979;
            } else {
                (*job).dstBuff = dstBuff;
...
unsafe extern "C" fn ZSTDMT_createJobsTable(
    ...
    let jobTable = ZSTD_customCalloc(
        (nbJobs as std::ffi::c_ulong)
            .wrapping_mul(
                ::core::mem::size_of::<ZSTDMT_jobDescription>() as std::ffi::c_ulong,
            ),
        cMem,
...
pub unsafe extern "C" fn ZSTDMT_sizeof_CCtx(mut mtctx: *mut ZSTDMT_CCtx) -> size_t {
    ...
    return (::core::mem::size_of::<ZSTDMT_CCtx>() as std::ffi::c_ulong)
...
unsafe extern "C" fn ZSTDMT_computeTargetJobLog(
    ...
    return if jobLog
        < (if MEM_32bits() != 0 { 29 as std::ffi::c_int } else { 30 as std::ffi::c_int })
            as std::ffi::c_uint
    {
        jobLog
    } else {
...
pub unsafe extern "C" fn ZSTDMT_initCStream_internal(
    ...
    let nbSlackBuffers = (2 as std::ffi::c_int
        + ((*mtctx).targetPrefixSize > 0 as std::ffi::c_int as size_t)
    ...
    if !dict.is_null() {
            ...
            == ZSTD_dct_rawContent as std::ffi::c_int as std::ffi::c_uint
        {
            (*mtctx)
                .inBuff
                .prefix
                .start = dict as *const BYTE as *const std::ffi::c_void;
    ...
    if (*mtctx).jobReady == 0 {
        ...
        *fresh4 = src as *const std::ffi::c_void;
        ...
        (*((*mtctx).jobs).offset(jobID as isize))
            .consumed = 0 as std::ffi::c_int as size_t;
        ...
        *fresh9 = &mut (*mtctx).serial;
        ...
        (*((*mtctx).jobs).offset(jobID as isize))
            ...
            as std::ffi::c_int as std::ffi::c_uint;
        (*((*mtctx).jobs).offset(jobID as isize))
            .dstFlushed = 0 as std::ffi::c_int as size_t;
...
unsafe extern "C" fn ZSTDMT_flushProduced(
        ...
        && (*((*mtctx).jobs).offset(wJobID as isize)).frameChecksumNeeded != 0
    {
        ...
        let ref mut fresh10 = (*((*mtctx).jobs).offset(wJobID as isize)).cSize;
    ...
    if cSize > 0 as std::ffi::c_int as size_t {
        ...
        let ref mut fresh11 = (*((*mtctx).jobs).offset(wJobID as isize)).dstFlushed;
...
unsafe extern "C" fn findSynchronizationPoint(
    ...
    while pos < syncPoint.toLoad {
        let toRemove = (if pos < RSYNC_LENGTH as size_t {
            ...
            *istart.offset(pos.wrapping_sub(RSYNC_LENGTH as size_t) as isize)
                as std::ffi::c_int
        }) as BYTE;
        ...
        if !((*mtctx).inBuff.buffer.start).is_null() {
            ...
            libc::memcpy(
                ((*mtctx).inBuff.buffer.start as *mut std::ffi::c_char)
                    .offset((*mtctx).inBuff.filled as isize) as *mut std::ffi::c_void,
