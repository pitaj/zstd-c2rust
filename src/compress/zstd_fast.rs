...
pub type ZSTD_match4Found = Option::<
    unsafe extern "C" fn(*const u8, *const u8, u32, u32) -> i32,
>;
...
pub const ZSTD_SHORT_CACHE_TAG_BITS: i32 = 8;
pub const ZSTD_SHORT_CACHE_TAG_MASK: u32 = ((1 as u32)
    << ZSTD_SHORT_CACHE_TAG_BITS)
    .wrapping_sub(1);
#[inline]
unsafe extern "C" fn ZSTD_writeTaggedIndex(
    hashTable: *mut u32,
    mut hashAndTag: usize,
    mut index: u32,
) {
...
unsafe extern "C" fn ZSTD_comparePackedTags(
    mut packedTag1: usize,
    mut packedTag2: usize,
) -> i32 {
...
unsafe extern "C" fn ZSTD_fillHashTableForCDict(
    ...
    let hBits = ((*cParams).hashLog)
        .wrapping_add(ZSTD_SHORT_CACHE_TAG_BITS as u32);
    ...
    while ip.offset(fastHashFillStep as isize)
        < iend.offset(2)
    {
        ...
        if !(dtlm as u32
            == ZSTD_dtlm_fast as i32 as u32)
        {
            let mut p: u32 = 0;
            p = 1;
            while p < fastHashFillStep {
                let hashAndTag_0 = ZSTD_hashPtr(
                    ip.offset(p as isize) as *const c_void,
                    hBits,
                    mls,
                );
                if *hashTable
...
unsafe extern "C" fn ZSTD_match4Found_cmov(
    ...
    let mut mvalAddr = ZSTD_selectAddr(
        matchIdx,
        idxLowLimit,
        matchAddress,
        dummy.as_ptr(),
    );
    if MEM_read32(currentPtr as *const c_void)
        != MEM_read32(mvalAddr as *const c_void)
    {
        return 0;
    }
    asm!("", options(preserves_flags, att_syntax));
    return (matchIdx >= idxLowLimit) as i32;
}
unsafe extern "C" fn ZSTD_match4Found_branch(
    ...
    if matchIdx >= idxLowLimit {
        ...
        mval = MEM_read32(currentPtr as *const c_void)
            ^ 1;
    }
    return (MEM_read32(currentPtr as *const c_void) == mval)
        as i32;
}
#[inline(always)]
unsafe extern "C" fn ZSTD_compressBlock_fast_noDict_generic(
    ...
    let stepSize = ((*cParams).targetLength)
        .wrapping_add(
            ((*cParams).targetLength == 0) as i32 as u32,
        )
    ...
    let kStepIncr = ((1 as i32) << kSearchStrength - 1 as i32)
        as usize;
    ...
    ip0 = ip0.offset((ip0 == prefixStart) as i32 as isize);
    ...
    '__start: loop {
        ...
        loop {
            let rval = MEM_read32(
                ip2.offset(-(rep_offset1 as isize)) as *const c_void,
            );
            current0 = ip0.offset_from(base) as std::ffi::c_long as u32;
            *hashTable.offset(hash0 as isize) = current0;
            if (MEM_read32(ip2 as *const c_void) == rval) as i32
                & (rep_offset1 > 0) as i32 != 0
            {
                ip0 = ip2;
                ...
                *hashTable
                ...
                current_block = 4052423780862933977;
                break;
            } else if matchFound
                .expect(
                    "non-null function pointer",
                )(ip0, base.offset(matchIdx as isize), matchIdx, prefixStartIndex) != 0
            {
                *hashTable
                    .offset(
                        hash1 as isize,
                    ) = ip1.offset_from(base) as std::ffi::c_long as u32;
                current_block = 17328913993171691972;
                break;
            } else {
                ...
                *hashTable.offset(hash0 as isize) = current0;
                if matchFound
                    .expect(
                        "non-null function pointer",
                    )(ip0, base.offset(matchIdx as isize), matchIdx, prefixStartIndex)
                    != 0
                {
                    if step <= 4 {
                        *hashTable
        ...
        ZSTD_storeSeq(
            seqStore,
            ip0.offset_from(anchor) as std::ffi::c_long as usize,
            anchor,
            iend,
            offcode,
            mLength,
            ...
            if rep_offset2 > 0 {
                while ip0 <= ilimit
                    && MEM_read32(ip0 as *const c_void)
                        == MEM_read32(
                            ip0.offset(-(rep_offset2 as isize))
                                as *const c_void,
                        )
                {
                    ...
                    *hashTable
                        .offset(
                            ZSTD_hashPtr(ip0 as *const c_void, hlog, mls)
                                as isize,
...
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_noDict_generic, cmov=1, ZSTD_compressBlock_fast_noDict_4_1);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_noDict_generic, cmov=1, ZSTD_compressBlock_fast_noDict_5_1);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_noDict_generic, cmov=1, ZSTD_compressBlock_fast_noDict_6_1);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_noDict_generic, cmov=1, ZSTD_compressBlock_fast_noDict_7_1);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_noDict_generic, cmov=0, ZSTD_compressBlock_fast_noDict_4_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_noDict_generic, cmov=0, ZSTD_compressBlock_fast_noDict_5_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_noDict_generic, cmov=0, ZSTD_compressBlock_fast_noDict_6_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_noDict_generic, cmov=0, ZSTD_compressBlock_fast_noDict_7_0);
#[no_mangle]
...
unsafe extern "C" fn ZSTD_compressBlock_fast_dictMatchState_generic(
    ...
    's_135: while ip1 <= ilimit {
        ...
        let dictHashAndTag0 = ZSTD_hashPtr(
            ip0 as *const c_void,
            dictHBits,
            mls,
        );
        ...
        let kStepIncr = ((1 as i32) << kSearchStrength) as usize;
        ...
        if ip0 <= ilimit {
            ...
            *hashTable
            ...
            while ip0 <= ilimit {
                ...
                *hashTable
                    .offset(
                        ZSTD_hashPtr(ip0 as *const c_void, hlog, mls) as isize,
                    ) = current2;
...
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_dictMatchState_generic, cmov=0, ZSTD_compressBlock_fast_dictMatchState_4_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_dictMatchState_generic, cmov=0, ZSTD_compressBlock_fast_dictMatchState_5_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_dictMatchState_generic, cmov=0, ZSTD_compressBlock_fast_dictMatchState_6_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_dictMatchState_generic, cmov=0, ZSTD_compressBlock_fast_dictMatchState_7_0);
#[no_mangle]
...
unsafe extern "C" fn ZSTD_compressBlock_fast_extDict_generic(
    ...
    '__start: loop {
        ...
        loop {
            ...
            let mut rval: u32 = 0;
            if (prefixStartIndex.wrapping_sub(repIndex) >= 4)
                as i32
                & (offset_1 > 0) as i32 != 0
            {
                rval = MEM_read32(
                    repBase.offset(repIndex as isize) as *const c_void,
                );
            } else {
                rval = MEM_read32(ip2 as *const c_void)
                    ^ 1;
            }
            current0 = ip0.offset_from(base) as std::ffi::c_long as u32;
            *hashTable.offset(hash0 as isize) = current0;
            if MEM_read32(ip2 as *const c_void) == rval {
                ...
                let mval = if idx >= dictStartIndex {
                    MEM_read32(idxBase.offset(idx as isize) as *const c_void)
                } else {
                    MEM_read32(ip0 as *const c_void)
                        ^ 1_u32
                };
                if MEM_read32(ip0 as *const c_void) == mval {
                    current_block = 15498112657170199191;
                    break;
                } else {
                    ...
                    *hashTable.offset(hash0 as isize) = current0;
                    let mval_0 = if idx >= dictStartIndex {
                        MEM_read32(
                            idxBase.offset(idx as isize) as *const c_void,
                        )
                    } else {
                        MEM_read32(ip0 as *const c_void)
                            ^ 1_u32
                    };
                    if MEM_read32(ip0 as *const c_void) == mval_0 {
                        current_block = 15498112657170199191;
                        break;
                    }
                    idx = *hashTable.offset(hash1 as isize);
...
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_extDict_generic, cmov=0, ZSTD_compressBlock_fast_extDict_4_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_extDict_generic, cmov=0, ZSTD_compressBlock_fast_extDict_5_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_extDict_generic, cmov=0, ZSTD_compressBlock_fast_extDict_6_0);
ZSTD_GEN_FAST_FN!(ZSTD_compressBlock_fast_extDict_generic, cmov=0, ZSTD_compressBlock_fast_extDict_7_0);
#[no_mangle]
