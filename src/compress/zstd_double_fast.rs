...
unsafe extern "C" fn ZSTD_fillDoubleHashTableForCDict(
    ...
    while ip.offset(fastHashFillStep as isize).offset(-1_isize)
        <= iend
    {
        ...
        while i < fastHashFillStep {
            let smHashAndTag = ZSTD_hashPtr(
                ip.offset(i as isize) as *const c_void,
                hBitsS,
                mls,
            );
    ...
    loop {
        's_428: {
            ...
            if !(ip1 > ilimit) {
                ...
                loop {
                    let hs0 = ZSTD_hashPtr(ip as *const c_void, hBitsS, mls);
                    ...
                    *hashLong.offset(hl0 as isize) = *fresh2;
                            ...
                            )) as i32 != 0
                    {
                        ...
                        ZSTD_storeSeq(
                            seqStore,
                            ip.offset_from(anchor) as std::ffi::c_long as usize,
                            anchor,
                            iend,
                            REPCODE1_TO_OFFBASE as u32,
                            mLength,
                        );
                        current_block_83 = 4253234386727138073;
                        break;
                    } else {
                        hl1 = ZSTD_hashPtr(
                            ip1 as *const c_void,
                            ...
                            && matchl0_safe == matchl0
                        {
                            ...
                            while (ip > anchor) as i32
                                & (matchl0 > prefixLowest) as i32 != 0
                                && *ip.offset(-1)
                                    as i32
                                    == *matchl0.offset(-1)
                                        as i32
                            ...
                            current_block_83 = 9285665692634906365;
                            break;
                        } else {
                            ...
                            if MEM_read32(matchs0_safe as *const c_void)
                                == MEM_read32(ip as *const c_void)
                                && matchs0_safe == matchs0
                            {
                                current_block_83 = 4601132248869953656;
                                break;
                            }
                            if ip1 >= nextStep {
                            ...
                            matchl0 = matchl1;
                            if !(ip1 <= ilimit) {
                                current_block_83 = 7117073317105758062;
                                break;
                            }
                        }
                    }
                    ...
                    _ => {
                        ...
                        if ip <= ilimit {
                            ...
                            *hashLong
                                .offset(
                                    ZSTD_hashPtr(
                                        base.offset(indexToInsert as isize)
                                            as *const c_void,
                                        hBitsL,
                                        8,
                                    ) as isize,
                            ...
                            *hashSmall
                                        ...
                                        )) as i32 != 0
                            {
                                ...
                                *hashSmall
                                    .offset(
                                        ZSTD_hashPtr(ip as *const c_void, hBitsS, mls)
                                            as isize,
                                    ) = ip.offset_from(base) as std::ffi::c_long as u32;
                                *hashLong
                                    .offset(
                                        ZSTD_hashPtr(
                                            ip as *const c_void,
...
unsafe extern "C" fn ZSTD_compressBlock_doubleFast_dictMatchState_generic(
    ...
    while ip < ilimit {
        ...
        let h = ZSTD_hashPtr(ip as *const c_void, hBitsS, mls);
        let dictHashAndTagL = ZSTD_hashPtr(
            ip as *const c_void,
            dictHBitsL,
            8,
        );
        let dictHashAndTagS = ZSTD_hashPtr(
            ip as *const c_void,
            dictHBitsS,
            mls,
        );
        ...
        let dictTagsMatchL = ZSTD_comparePackedTags(
            dictMatchIndexAndTagL as usize,
            dictHashAndTagL,
        );
        let dictTagsMatchS = ZSTD_comparePackedTags(
            dictMatchIndexAndTagS as usize,
            dictHashAndTagS,
        );
        let curr = ip.offset_from(base) as std::ffi::c_long as u32;
        ...
        let ref mut fresh7 = *hashSmall.offset(h as isize);
        *fresh7 = curr;
        *hashLong.offset(h2 as isize) = *fresh7;
                ...
                )
        {
            ...
            if matchIndexL >= prefixLowestIndex
                && MEM_read64(matchLong as *const c_void)
                    == MEM_read64(ip as *const c_void)
            {
                mLength = (ZSTD_count(
                    ip.offset(8),
                ...
                while (ip > anchor) as i32
                    & (matchLong > prefixLowest) as i32 != 0
                    && *ip.offset(-1) as i32
                        == *matchLong.offset(-1)
                            as i32
                {
                ...
                if dictTagsMatchL != 0 {
                    let dictMatchIndexL = dictMatchIndexAndTagL
                        >> ZSTD_SHORT_CACHE_TAG_BITS;
                            ...
                            == MEM_read64(ip as *const c_void)
                    {
                        ...
                        current_block = 4044895159565194570;
                    } else {
                    ...
                    _ => {
                        if matchIndexS > prefixLowestIndex {
                            ...
                            if match_0 > dictStart
                                && MEM_read32(match_0 as *const c_void)
                                    == MEM_read32(ip as *const c_void)
                            {
                                current_block = 2631791190359682872;
                            } else {
                                    ...
                                    if dictTagsMatchL3 != 0 {
                                                ...
                                                )
                                        {
                                            ...
                                            offset = curr
                                                ...
                                                .wrapping_sub(dictIndexDelta);
                                            while (ip > anchor) as i32
                                                & (dictMatchL3 > dictStart) as i32 != 0
                                                && *ip.offset(-1)
                                                    as i32
        ...
        if ip <= ilimit {
            ...
            *hashLong
                .offset(
                    ZSTD_hashPtr(
                        base.offset(indexToInsert as isize) as *const c_void,
                        hBitsL,
                        8,
                    ) as isize,
                ) = indexToInsert;
            *hashLong
                ...
                ) = ip.offset(-2_isize).offset_from(base)
            ...
            *hashSmall
                ...
                ) = ip.offset(-1_isize).offset_from(base)
                as std::ffi::c_long as u32;
            while ip <= ilimit {
                ...
                *hashSmall
                    .offset(
                        ZSTD_hashPtr(ip as *const c_void, hBitsS, mls) as isize,
                    ) = current2;
                *hashLong
                    .offset(
                        ZSTD_hashPtr(
...
unsafe extern "C" fn ZSTD_compressBlock_doubleFast_extDict_generic(
    ...
    while ip < ilimit {
        let hSmall = ZSTD_hashPtr(ip as *const c_void, hBitsS, mls);
        ...
        let mut mLength: usize = 0;
        let ref mut fresh8 = *hashLong.offset(hLong as isize);
        *fresh8 = curr;
        *hashSmall.offset(hSmall as isize) = *fresh8;
        if ZSTD_index_overlap_check(prefixStartIndex, repIndex)
            & (offset_1
                <= curr
                    .wrapping_add(1)
                    .wrapping_sub(dictStartIndex)) as i32 != 0
            && MEM_read32(repMatch as *const c_void)
                == MEM_read32(
                    ip.offset(1) as *const c_void,
                )
                ...
                == MEM_read64(ip as *const c_void)
        {
            ...
            while (ip > anchor) as i32
                & (matchLong > lowMatchPtr) as i32 != 0
                && *ip.offset(-1) as i32
                    == *matchLong.offset(-1)
                        as i32
            {
                ...
                == MEM_read32(ip as *const c_void)
        {
            ...
            *hashLong
                .offset(h3 as isize) = curr.wrapping_add(1);
                    ...
                    )
            {
                ...
                while (ip > anchor) as i32
                    & (match_0 > lowMatchPtr_1) as i32 != 0
                    && *ip.offset(-1) as i32
                        == *match_0.offset(-1)
                            as i32
                {
