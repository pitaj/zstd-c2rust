...
unsafe extern "C" fn ZSTD_fillDoubleHashTableForCDict(
    ...
    while ip.offset(fastHashFillStep as isize).offset(-(1 as std::ffi::c_int as isize))
        <= iend
    {
...
unsafe extern "C" fn ZSTD_compressBlock_doubleFast_noDict_generic(
    ...
    let dummy: [u8; 10] = [
        ...
        0x9a as std::ffi::c_int as u8,
        0xbc as std::ffi::c_int as u8,
        0xde as std::ffi::c_int as u8,
        0xf0 as std::ffi::c_int as u8,
        0xe2 as std::ffi::c_int as u8,
        0xb4 as std::ffi::c_int as u8,
    ];
    ip = ip
        .offset(
    ...
    loop {
        's_428: {
            ...
            step = 1 as std::ffi::c_int as usize;
            ...
            if !(ip1 > ilimit) {
                ...
                loop {
                    ...
                    *hashLong.offset(hl0 as isize) = *fresh2;
                            ...
                            )) as std::ffi::c_int != 0
                    {
                        ...
                        current_block_83 = 6083028660245302711;
                        break;
                    } else {
                        hl1 = ZSTD_hashPtr(
                            ...
                            && matchl0_safe == matchl0
                        {
                            ...
                            current_block_83 = 6399858183300489367;
                            break;
                        } else {
                            ...
                            {
                                current_block_83 = 12286646964635847727;
                                break;
                            }
                            if ip1 >= nextStep {
                            ...
                            matchl0 = matchl1;
                            if !(ip1 <= ilimit) {
                                current_block_83 = 14991511065822551758;
                                break;
                            }
                        }
                    }
                ...
                match current_block_83 {
                    14991511065822551758 => {}
                    _ => {
                        match current_block_83 {
                            12286646964635847727 => {
                                mLength = (ZSTD_count(
                        ...
                        if ip <= ilimit {
                            let indexToInsert = curr
                                .wrapping_add(2 as std::ffi::c_int as u32);
                            *hashLong
                                .offset(
                                    ZSTD_hashPtr(
...
unsafe extern "C" fn ZSTD_compressBlock_doubleFast_dictMatchState_generic(
    ...
    if (*ms).prefetchCDictTables != 0 {
        ...
        let chainTableBytes = ((1 as std::ffi::c_int as usize)
        ...
        _pos_0 = 0 as std::ffi::c_int as usize;
        ...
        *hashLong.offset(h2 as isize) = *fresh3;
                    ...
                    == MEM_read64(ip as *const std::ffi::c_void)
            {
                ...
                if dictTagsMatchL != 0 {
                            ...
                            == MEM_read64(ip as *const std::ffi::c_void)
                    {
                        ...
                        current_block = 18089769140019963903;
                    } else {
        ...
        if ip <= ilimit {
            ...
            *hashLong
                ...
                ) = ip.offset(-(2 as std::ffi::c_int as isize)).offset_from(base)
            ...
            *hashSmall
                ...
                ) = ip.offset(-(1 as std::ffi::c_int as isize)).offset_from(base)
                as std::ffi::c_long as u32;
            while ip <= ilimit {
        ...
        *hashSmall.offset(hSmall as isize) = *fresh4;
        if ZSTD_index_overlap_check(prefixStartIndex, repIndex)
            & (offset_1
                <= curr
                    .wrapping_add(1 as std::ffi::c_int as u32)
                    .wrapping_sub(dictStartIndex)) as std::ffi::c_int != 0
            && MEM_read32(repMatch as *const std::ffi::c_void)
                == MEM_read32(
                    ip.offset(1 as std::ffi::c_int as isize) as *const std::ffi::c_void,
                )
                ...
                == MEM_read32(ip as *const std::ffi::c_void)
        {
            ...
            *hashLong
                .offset(h3 as isize) = curr.wrapping_add(1 as std::ffi::c_int as u32);
