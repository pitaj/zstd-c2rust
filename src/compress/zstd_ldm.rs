...
pub const LDM_BATCH_SIZE: i32 = 64;
...
unsafe extern "C" fn ZSTD_window_hasExtDict(window: ZSTD_window_t) -> u32 {
    return (window.lowLimit < window.dictLimit) as i32 as u32;
}
#[inline]
unsafe extern "C" fn ZSTD_matchState_dictMode(
    ...
    return (if ZSTD_window_hasExtDict((*ms).window) != 0 {
        ...
        if (*(*ms).dictMatchState).dedicatedDictSearch != 0 {
            ZSTD_dedicatedDictSearch as i32
        } else {
            ZSTD_dictMatchState as i32
        }
    } else {
        ZSTD_noDict as i32
    }) as ZSTD_dictMode_e;
...
unsafe extern "C" fn ZSTD_window_canOverflowCorrect(
    ...
    let indexLargeEnough = (curr > adjustedIndex) as i32 as u32;
    let dictionaryInvalidated = (curr > maxDist.wrapping_add(loadedDictEnd))
        as i32 as u32;
    return (indexLargeEnough != 0 && dictionaryInvalidated != 0) as i32
        as u32;
}
#[inline]
...
pub const LDM_MIN_MATCH_LENGTH: i32 = 64;
    ...
    let totalSize = (ZSTD_cwksp_alloc_size(ldmBucketSize))
        .wrapping_add(
            ZSTD_cwksp_alloc_size(
                ldmHSize
                    .wrapping_mul(
                        ::core::mem::size_of::<ldmEntry_t>(),
                    ),
            ),
        );
    return if params.enableLdm as u32
...
unsafe extern "C" fn ZSTD_ldm_getBucket(
    ...
    return ((*ldmState).hashTable).offset((hash << bucketSizeLog) as isize);
}
...
unsafe extern "C" fn ZSTD_ldm_generateSequences_internal(
    ...
    let minMatchLength = (*params).minMatchLength;
    let entsPerBucket = (1 as u32) << (*params).bucketSizeLog;
    ...
    while ip < ilimit {
        ...
        hashed = ZSTD_ldm_gear_feed(
            &mut hashState,
            ip,
            ilimit.offset_from(ip) as std::ffi::c_long as usize,
            splits,
            &mut numSplits,
        );
        n = 0;
            ...
            if split_0 < anchor {
                ...
                if bestEntry.is_null() {
                    ...
                    (*seq)
                        ...
                        .offset_from(anchor) as std::ffi::c_long as u32;
...
unsafe extern "C" fn ZSTD_ldm_reduceTable(
    ...
    while u < size {
        if (*table.offset(u as isize)).offset < reducerValue {
            ...
            let ref mut fresh34 = (*table.offset(u as isize)).offset;
            *fresh34 = (*fresh34).wrapping_sub(reducerValue);
        }
        u = u.wrapping_add(1);
        u;
    }
...
pub unsafe extern "C" fn ZSTD_ldm_generateSequences(
    ...
    let istart = src as *const u8;
    let iend = istart.offset(srcSize as isize);
    let kMaxChunkSize = ((1 as i32) << 20) as usize;
    ...
    while chunk < nbChunks && (*sequences).size < (*sequences).capacity {
        ...
        let remaining = iend.offset_from(chunkStart) as std::ffi::c_long as usize;
        let chunkEnd = if remaining < kMaxChunkSize {
            iend
        } else {
            chunkStart.offset(kMaxChunkSize as isize)
        };
        let chunkSize = chunkEnd.offset_from(chunkStart) as std::ffi::c_long as usize;
        ...
        if prevSize < (*sequences).size {
            let ref mut fresh35 = (*((*sequences).seq).offset(prevSize as isize))
                .litLength;
            *fresh35 = (*fresh35).wrapping_add(leftoverSize as u32);
        ...
        if srcSize < (*seq).matchLength as usize {
            ...
            if (*seq).matchLength < minMatch {
                if ((*rawSeqStore).pos).wrapping_add(1)
                    < (*rawSeqStore).size
                {
                    let ref mut fresh36 = (*seq.offset(1))
                        .litLength;
                    *fresh36 = (*fresh36)
                        .wrapping_add(
...
pub unsafe extern "C" fn ZSTD_ldm_blockCompress(
    ...
    while (*rawSeqStore).pos < (*rawSeqStore).size && ip < iend {
        ...
        i = ZSTD_REP_NUM - 1;
        while i > 0 {
            *rep.offset(i as isize) = *rep.offset((i - 1 as i32) as isize);
            i -= 1;
            i;
        }
