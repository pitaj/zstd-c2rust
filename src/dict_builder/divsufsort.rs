        ...
        if limit < 0 as std::ffi::c_int {
            if limit == -(1 as std::ffi::c_int) {
                    ...
                    < b.offset_from(a) as std::ffi::c_long
                {
                    ...
                    stack[ssize as usize].d = 0 as std::ffi::c_int;
                    let fresh55 = ssize;
                    ssize = ssize + 1;
                    stack[fresh55 as usize].e = 0 as std::ffi::c_int;
                    if ssize < 64 as std::ffi::c_int {} else {
                        __assert_fail(
                            b"ssize < STACK_SIZE\0" as *const u8
                                as *const std::ffi::c_char,
    ...
    while 0 as std::ffi::c_int <= i {
        ...
        if 0 as std::ffi::c_int <= i {
            let ref mut fresh81 = *bucket_B
                .offset((c0 << 8 as std::ffi::c_int | c1) as isize);
            *fresh81 += 1;
            *fresh81;
            m -= 1;
            *SA.offset(m as isize) = i;
                ...
                }
            {
                let ref mut fresh82 = *bucket_B
                    .offset((c1 << 8 as std::ffi::c_int | c0) as isize);
                *fresh82 += 1;
                *fresh82;
                i -= 1;
                i;
    ...
    if (0 as std::ffi::c_int) < m {
        ...
        *bucket_B
        ...
        while 0 as std::ffi::c_int <= c0 {
            ...
            *bucket_B
                .offset(
                    (c0 << 8 as std::ffi::c_int | c0 + 1 as std::ffi::c_int) as isize,
                ) = i - *bucket_B.offset((c0 << 8 as std::ffi::c_int | c0) as isize)
                + 1 as std::ffi::c_int;
        ...
        while 0 as std::ffi::c_int <= c1 {
            i = SA
                .offset(
                    *bucket_B
                        .offset(
                            (c1 << 8 as std::ffi::c_int | c1 + 1 as std::ffi::c_int)
                                as isize,
                        ) as isize,
                );
            j = SA
                    ...
                    *j = !s;
                    ...
                    if c0 != c2 {
                        if 0 as std::ffi::c_int <= c2 {
                            *bucket_B
                                .offset(
                                    (c1 << 8 as std::ffi::c_int | c2) as isize,
