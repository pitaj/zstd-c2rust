...
extern "C" {
    ...
    fn ZSTD_seqToCodes(seqStorePtr: *const SeqStore_t) -> i32;
...
pub const MINRATIO: i32 = 4;
pub const ZDICT_MAX_SAMPLES_SIZE: u32 = (2000 as u32)
    << 20;
pub const ZDICT_MIN_SAMPLES_SIZE: i32 = ZDICT_CONTENTSIZE_MIN * MINRATIO;
use crate::common::mem::*;
#[inline]
...
pub const MLFSELog: i32 = 9;
pub const LLFSELog: i32 = 9;
pub const OffFSELog: i32 = 8;
pub const ZDICT_DICTSIZE_MIN: i32 = 256;
pub const ZDICT_CONTENTSIZE_MIN: i32 = 128;
pub const CLOCKS_PER_SEC: i32 = 1000000;
pub const NULL: i32 = 0;
pub const NOISELENGTH: i32 = 32;
...
unsafe extern "C" fn ZDICT_printHex(
    ...
    while u < length {
        ...
        DISPLAY!(
            "%c", c
        )(stderr, b"%c\0" as *const u8 as *const c_char, c as i32);
        DISPLAY!("%c", c)(stderr);
        u = u.wrapping_add(1);
        u;
    }
...
pub unsafe extern "C" fn ZDICT_getDictID(
    ...
    return MEM_readLE32(
        (dictBuffer as *const c_char).offset(4)
            as *const c_void,
    );
}
#[no_mangle]
pub unsafe extern "C" fn ZDICT_getDictHeaderSize(
    ...
    libc::free(bs as *mut c_void);
    libc::free(wksp as *mut c_void);
    return headerSize;
}
unsafe extern "C" fn ZDICT_count(
    mut pIn: *const c_void,
    mut pMatch: *const c_void,
) -> usize {
    ...
    loop {
        ...
        if diff == 0 {
            pIn = (pIn as *const c_char)
                ...
                as *const c_void;
            pMatch = (pMatch as *const c_char)
                .offset(::core::mem::size_of::<usize>() as isize)
                as *const c_void;
        } else {
            pIn = (pIn as *const c_char)
                .offset(ZSTD_NbCommonBytes(diff) as isize) as *const c_void;
            return (pIn as *const c_char).offset_from(pStart)
...
pub const LLIMIT: i32 = 64;
pub const MINMATCHLENGTH: i32 = 7;
unsafe extern "C" fn ZDICT_analyzePos(
    mut doneMarks: *mut u8,
    mut suffix: *const u32,
    mut start: u32,
    ...
    *doneMarks.offset(pos as isize) = 1;
    if MEM_read16(
        b.offset(pos as isize).offset(0)
            as *const c_void,
    ) as i32
        == MEM_read16(
            ...
            ) as i32
    {
        ...
        while MEM_read16(
            b.offset(pos as isize).offset(patternEnd as isize) as *const c_void,
        ) as i32 == pattern16 as i32
        {
            patternEnd = patternEnd.wrapping_add(2);
        }
        if *b.offset(pos.wrapping_add(patternEnd as usize) as isize) as i32
            == *b
                .offset(
                    pos
    ...
    loop {
        ...
        length = ZDICT_count(
            b.offset(pos as isize) as *const c_void,
    ...
    loop {
        ...
        if length_1 >= LLIMIT as usize {
            length_1 = (LLIMIT - 1 as i32) as usize;
        }
    ...
    while (length_2 >= MINMATCHLENGTH as usize) as i32
        & (start > 0) as i32 != 0
    {
        length_2 = ZDICT_count(
            ...
            b
                .offset(
                    *suffix
                        .offset(start.wrapping_sub(1) as isize)
                        as isize,
                ) as *const c_void,
        );
        if length_2 >= LLIMIT as usize {
            length_2 = (LLIMIT - 1 as i32) as usize;
        }
    ...
    cumulLength[maxLength.wrapping_sub(1)
        ...
        as usize];
    i = maxLength.wrapping_sub(2) as i32;
    while i >= 0 {
        cumulLength[i
            as usize] = (cumulLength[(i + 1 as i32) as usize])
            .wrapping_add(lengthList[i as usize]);
        i -= 1;
        i;
    }
    let mut u_0: u32 = 0;
    u_0 = (LLIMIT - 1 as i32) as u32;
    while u_0 >= MINMATCHLENGTH as u32 {
    ...
    let c = *b
        .offset(
            pos.wrapping_add(maxLength).wrapping_sub(1)
                as isize,
        );
    while *b
        .offset(
            pos.wrapping_add(l as usize).wrapping_sub(2)
                as isize,
        ) as i32 == c as i32
    {
        l = l.wrapping_sub(1);
    ...
    while id_0 < end {
        ...
        if testedPos as usize == pos {
            ...
            length_3 = ZDICT_count(
                b.offset(pos as isize) as *const c_void,
                b.offset(testedPos as isize) as *const c_void,
            ) as u32;
...
unsafe extern "C" fn isIncluded(
    mut in_0: *const c_void,
    mut container: *const c_void,
    mut length: usize,
) -> i32 {
    let ip = in_0 as *const c_char;
    let into = container as *const c_char;
    let mut u: usize = 0;
    u = 0;
    while u < length {
        if *ip.offset(u as isize) as i32
            != *into.offset(u as isize) as i32
        {
            break;
        }
        u = u.wrapping_add(1);
    ...
    while u < tableSize {
        if !(u == eltNbToSkip) {
            if ((*table.offset(u as isize)).pos)
                ...
                && (*table.offset(u as isize)).pos < elt.pos
            {
                let addedLength_0 = eltEnd as i32
                ...
                *fresh3 = (*fresh3)
                ...
                if addedLength_0 > 0 {
                    ...
                    *fresh4 = (*fresh4 as u32)
                        .wrapping_add(addedLength_0 as u32) as u32 as u32;
                    let ref mut fresh5 = (*table.offset(u as isize)).savings;
                    *fresh5 = (*fresh5 as u32)
                        .wrapping_add(
                            (elt.savings)
...
unsafe extern "C" fn ZDICT_removeDictItem(mut table: *mut dictItem, mut id: u32) {
    ...
    while u < max.wrapping_sub(1) {
        *table
            .offset(
                u as isize,
            ) = *table.offset(u.wrapping_add(1) as isize);
        u = u.wrapping_add(1);
        u;
    }
...
unsafe extern "C" fn ZDICT_insertDictItem(
    ...
    while (*table.offset(current as isize)).savings < elt.savings {
        *table
            .offset(
                current.wrapping_add(1) as isize,
...
unsafe extern "C" fn ZDICT_trainBuffer_legacy(
        ...
        || filePos.is_null()
    {
        ...
        if minRatio < MINRATIO as u32 {
            minRatio = MINRATIO as u32;
        }
        libc::memset(
            doneMarks as *mut c_void,
            0,
        ...
        if divSuftSortResult != 0 {
            ...
            *suffix.offset(bufferSize as isize) = bufferSize as u32;
            *suffix0
                .offset(0) = bufferSize as u32;
            let mut pos: usize = 0;
            pos = 0;
            ...
            *filePos.offset(0) = 0;
            ...
            while pos < nbFiles as usize {
                *filePos
                    ...
                    .offset(pos.wrapping_sub(1) as isize)
                    as usize)
                    .wrapping_add(
                        *fileSizes
                            .offset(
                        ...
                        ) >= 2
                        {
                            ...
                            ) != 0
                            {
                                ...
                                *fresh7 = DISPLAYUPDATE!(
                                ...
                                fprintf(
                                    stderr,
                                    b"\r%4.2f %% \r\0" as *const u8 as *const c_char,
    ...
    libc::free(suffix0 as *mut c_void);
    libc::free(reverseSuffix as *mut c_void);
    libc::free(doneMarks as *mut c_void);
    libc::free(filePos as *mut c_void);
    return result;
}
...
pub const MAXREPOFFSET: i32 = 1024;
unsafe extern "C" fn ZDICT_countEStats(
    ...
    if cSize != 0 {
        ...
        if nbSeq >= 2 {
            ...
            *fresh12 = (*fresh12).wrapping_add(3);
...
unsafe extern "C" fn ZDICT_insertSortCount(
    ...
    while u > 0 {
        ...
        if (*table.offset(u.wrapping_sub(1) as isize)).count
...
unsafe extern "C" fn ZDICT_analyzeEntropy(
    ...
    let mut offcodeNCount: [i16; 31] = [0; 31];
    ...
    let averageSampleSize = totalSrcSize
        / nbFiles.wrapping_add((nbFiles == 0) as i32 as u32)
            as usize;
        ...
        if (esr.dict).is_null() || (esr.zc).is_null() || (esr.workPlace).is_null() {
            ...
            while u < nbFiles {
                ZDICT_countEStats(
                    ...
                    (srcBuffer as *const c_char).offset(pos as isize)
                        as *const c_void,
...
pub unsafe extern "C" fn ZDICT_finalizeDictionary(
    ...
    mut customDictContent: *const c_void,
    ...
    let eSize = ZDICT_analyzeEntropy(
        header.as_mut_ptr().offset(hSize as isize) as *mut c_void,
    ...
    libc::memmove(outDictContent as *mut c_void, customDictContent, dictContentSize);
    libc::memcpy(
        outDictHeader as *mut c_void,
        header.as_mut_ptr() as *const c_void,
        hSize,
    );
    libc::memset(outDictPadding as *mut c_void, 0, paddingSize);
    return dictSize;
}
pub const HBUFFSIZE: i32 = 256;
unsafe extern "C" fn ZDICT_addEntropyTablesFromBuffer_advanced(
    ...
    DISPLAYLEVEL!(2, "statistics ... \n");
    let eSize = ZDICT_analyzeEntropy(
        (dictBuffer as *mut c_char).offset(hSize as isize)
            as *mut c_void,
        ...
        (dictBuffer as *mut c_char)
            ...
            .offset(-(dictContentSize as isize)) as *const c_void,
        dictContentSize,
        notificationLevel,
    );
    if ERR_isError(eSize) {
    ...
    let dictID = if params.dictID != 0 { params.dictID } else { compliantID };
    MEM_writeLE32(
        (dictBuffer as *mut c_char).offset(4)
            as *mut c_void,
        dictID,
    );
    if hSize.wrapping_add(dictContentSize) < dictBufferCapacity {
...
unsafe extern "C" fn ZDICT_trainFromBuffer_unsafe_legacy(
    ...
    let dictListSize = if (if 10000 as u32 > nbSamples {
        10000 as u32
    } else {
        nbSamples
    }) > (maxDictSize / 16_usize) as u32
    {
        if 10000 as u32 > nbSamples {
            10000 as u32
        } else {
            nbSamples
        }
    ...
    if maxDictSize < ZDICT_DICTSIZE_MIN as usize {
        libc::free(dictList as *mut c_void);
    ...
    if params.zParams.notificationLevel >= 3 {
        ...
        while u < nb {
            ...
            ZDICT_printHex(
                (samplesBuffer as *const c_char).offset(pos as isize)
                    as *const c_void,
    ...
    if dictContentSize_0 < ZDICT_CONTENTSIZE_MIN as u32 {
        libc::free(dictList as *mut c_void);
        return ERROR(ZSTD_error_dictionaryCreation_failed);
    }
        ...
        if samplesBuffSize < 10 * targetDictSize {
            if notificationLevel >= 2 {
                fprintf(
                    ...
                    (samplesBuffSize >> 20) as u32,
                );
                fflush(stderr);
            }
        }
        if minRep > MINRATIO as u32 {
    ...
    if dictContentSize_0 as usize > targetDictSize * 3_usize
        && nbSamples > (2 as i32 * MINRATIO) as u32
        && selectivity > 1
    {
        let mut proposedSelectivity = selectivity
            .wrapping_sub(1);
        while nbSamples >> proposedSelectivity <= MINRATIO as u32 {
...
pub unsafe extern "C" fn ZDICT_trainFromBuffer_legacy(
    ...
    ZDICT_fillNoise(
        (newBuff as *mut c_char).offset(sBuffSize as isize)
            as *mut c_void,
