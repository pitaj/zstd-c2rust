...
pub const FASTCOVER_MAX_F: std::ffi::c_int = 31 as std::ffi::c_int;
pub const FASTCOVER_MAX_ACCEL: std::ffi::c_int = 10 as std::ffi::c_int;
pub const FASTCOVER_DEFAULT_SPLITPOINT: std::ffi::c_double = 0.75f64;
pub const DEFAULT_F: std::ffi::c_int = 20 as std::ffi::c_int;
...
static mut FASTCOVER_defaultAccelParameters: [FASTCOVER_accel_t; 11] = [
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 100 as std::ffi::c_int as std::ffi::c_uint,
            skip: 0 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
    ...
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 50 as std::ffi::c_int as std::ffi::c_uint,
            skip: 1 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 34 as std::ffi::c_int as std::ffi::c_uint,
            skip: 2 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 25 as std::ffi::c_int as std::ffi::c_uint,
            skip: 3 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 20 as std::ffi::c_int as std::ffi::c_uint,
            skip: 4 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 17 as std::ffi::c_int as std::ffi::c_uint,
            skip: 5 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 14 as std::ffi::c_int as std::ffi::c_uint,
            skip: 6 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 13 as std::ffi::c_int as std::ffi::c_uint,
            skip: 7 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 11 as std::ffi::c_int as std::ffi::c_uint,
            skip: 8 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
    {
        let mut init = FASTCOVER_accel_t {
            finalize: 10 as std::ffi::c_int as std::ffi::c_uint,
            skip: 9 as std::ffi::c_int as std::ffi::c_uint,
        };
        init
    },
];
unsafe extern "C" fn FASTCOVER_selectSegment(
    ...
    while pos != bestSegment.end {
        ...
        *freqs.offset(i as isize) = 0 as std::ffi::c_int as U32;
...
unsafe extern "C" fn FASTCOVER_computeFrequency(
    ...
    let readLength = if d > 8 as std::ffi::c_int as std::ffi::c_uint {
        d
    } else {
...
unsafe extern "C" fn FASTCOVER_ctx_init(
    ...
    let mut i: U32 = 0;
    *((*ctx).offsets)
        .offset(0 as std::ffi::c_int as isize) = 0 as std::ffi::c_int as size_t;
...
unsafe extern "C" fn FASTCOVER_buildDictionary(
    ...
    let maxZeroScoreRun = 10 as std::ffi::c_int as size_t;
    ...
    while tail > 0 as std::ffi::c_int as size_t {
        ...
        if segment.score == 0 as std::ffi::c_int as U32 {
            ...
            segmentSize = if ((segment.end)
                ...
                .wrapping_sub(1 as std::ffi::c_int as std::ffi::c_uint) as size_t) < tail
            {
                (segment.end)
                    .wrapping_sub(segment.begin)
                    .wrapping_add(parameters.d)
                    .wrapping_sub(1 as std::ffi::c_int as std::ffi::c_uint) as size_t
            } else {
                tail
            };
                    ...
                    || displayLevel >= 4 as std::ffi::c_int
                {
                    ...
                    fprintf(
                        ...
                        (dictBufferCapacity.wrapping_sub(tail)
                            * 100 as std::ffi::c_int as size_t / dictBufferCapacity)
                            as std::ffi::c_uint,
                    );
                    fflush(stderr);
                }
        ...
        epoch = epoch.wrapping_add(1 as std::ffi::c_int as size_t)
            % epochs.num as size_t;
    }
    if displayLevel >= 2 as std::ffi::c_int {
        fprintf(
            stderr,
...
unsafe extern "C" fn FASTCOVER_tryParameters(mut opaque: *mut std::ffi::c_void) {
    ...
    if segmentFreqs.is_null() || dict.is_null() || freqs.is_null() {
        ...
        let nbFinalizeSamples = ((*ctx).nbTrainSamples
            * (*ctx).accelParams.finalize as size_t / 100 as std::ffi::c_int as size_t)
            as std::ffi::c_uint;
...
pub unsafe extern "C" fn ZDICT_trainFromBuffer_fastCover(
    ...
    let nbFinalizeSamples = (ctx.nbTrainSamples * ctx.accelParams.finalize as size_t
        / 100 as std::ffi::c_int as size_t) as std::ffi::c_uint;
