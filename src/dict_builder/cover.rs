...
unsafe extern "C" fn COVER_lower_bound(
    ...
    while count != 0 as std::ffi::c_int as size_t {
        ...
        if *ptr < value {
            ...
            first = ptr;
            count = count
                .wrapping_sub(step.wrapping_add(1 as std::ffi::c_int as size_t));
        } else {
            count = step;
        }
    }
...
unsafe extern "C" fn COVER_ctx_init(
    ...
    if nbTrainSamples < 5 as std::ffi::c_int as std::ffi::c_uint {
        if DISPLAYLEVEL!(
            1, "Total number of training samples is %u and is invalid.", nbTrainSamples
        ) >= 1 as std::ffi::c_int
        {
            fprintf(
                stderr,
    ...
    if nbTestSamples < 1 as std::ffi::c_int as std::ffi::c_uint {
        if DISPLAYLEVEL!(
            1, "Total number of testing samples is %u and is invalid.", nbTestSamples
        ) >= 1 as std::ffi::c_int
        {
            fprintf(
                stderr,
    ...
    *((*ctx).offsets)
    ...
    if DISPLAYLEVEL!(2, "Constructing partial suffix array\n") >= 2 as std::ffi::c_int {
        fprintf(
            stderr,
...
pub unsafe extern "C" fn COVER_warnOnSmallCorpus(
    ...
    if DISPLAYLEVEL!(
        1, "WARNING: The maximum dictionary size %u is too large "
...
pub unsafe extern "C" fn ZDICT_trainFromBuffer_cover(
    ...
    if COVER_checkParameters(parameters, dictBufferCapacity) == 0 {
        if DISPLAYLEVEL!(1, "Cover parameters incorrect\n") >= 1 as std::ffi::c_int {
            fprintf(
                stderr,
...
pub unsafe extern "C" fn COVER_dictSelectionError(
    ...
    return setDictSelection(NULL as *mut BYTE, 0 as std::ffi::c_int as size_t, error);
}
#[no_mangle]
...
pub unsafe extern "C" fn COVER_selectDict(
    ...
    while dictContentSize < largestDict {
        ...
        dictContentSize = dictContentSize * 2 as std::ffi::c_int as size_t;
    }
