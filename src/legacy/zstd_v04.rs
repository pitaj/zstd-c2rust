...
unsafe extern "C" fn ZSTD_getFrameParams(
    ...
    (*params)
        .windowLog = ((*(src as *const u8).offset(4)
        as libc::c_int & 15) + ZSTD_WINDOWLOG_ABSOLUTEMIN) as u32;
    if *(src as *const u8).offset(4) as libc::c_int
...
pub unsafe extern "C" fn ZSTDv04_findFrameSizeInfoLegacy(
    ...
    remainingSize = (remainingSize as libc::c_ulong)
        .wrapping_sub(ZSTD_frameHeaderSize_min) ;
    loop {
        let mut cBlockSize = ZSTD_getcBlockSize(
            ip as *const libc::c_void,
            remainingSize,
...
unsafe extern "C" fn ZBUFF_createDCtx() -> *mut ZBUFF_DCtx {
    let mut zbc = malloc(::core::mem::size_of::<ZBUFF_DCtx>())
        as *mut ZBUFF_DCtx;
    if zbc.is_null() {
