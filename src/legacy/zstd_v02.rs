...
pub const ZSTD_magicNumber: std::ffi::c_uint = 0xfd2fb522 as std::ffi::c_uint;
...
unsafe extern "C" fn HUF_fillDTableX6LevelN(
    ...
    let scaleLog = nbBitsBaseline.wrapping_sub(sizeLog as U32) as std::ffi::c_int;
    let minBits = nbBitsBaseline.wrapping_sub(maxWeight) as std::ffi::c_int;
    ...
    memcpy(
        rankVal.as_mut_ptr() as *mut std::ffi::c_void,
        (*rankValOrigin.offset(consumed as isize)).as_ptr() as *const std::ffi::c_void,
        ::core::mem::size_of::<[U32; 17]>() as std::ffi::c_ulong,
    );
    if minWeight > 1 as std::ffi::c_int {
        let mut i: U32 = 0;
    ...
    while s < sortedListSize {
        ...
        let weight = (*sortedSymbols.offset(s as isize)).weight as U32;
        let nbBits = nbBitsBaseline.wrapping_sub(weight) as std::ffi::c_int;
        let totalBits = consumed.wrapping_add(nbBits as U32) as std::ffi::c_int;
        let start = rankVal[weight as usize];
        let length = ((1 as std::ffi::c_int) << sizeLog - nbBits) as U32;
        baseSeq.byte[level as usize] = symbol;
        DDesc.nbBits = totalBits as BYTE;
        if level < 3 as std::ffi::c_int as U32 && sizeLog - totalBits >= minBits {
            let mut nextMinWeight = totalBits + scaleLog;
            if nextMinWeight < 1 as std::ffi::c_int {
                nextMinWeight = 1 as std::ffi::c_int;
            }
...
unsafe extern "C" fn HUF_readDTableX6(
    ...
    *rankStart.offset(0 as std::ffi::c_int as isize) = 0 as std::ffi::c_int as U32;
    ...
    let mut dSeqStart = DTable
        ...
        .offset(
            ((1 as std::ffi::c_int as size_t)
                << memLog.wrapping_sub(1 as std::ffi::c_int as U32)) as isize,
    ...
    DSeq.sequence = 0 as std::ffi::c_int as U32;
    DDesc.nbBits = 0 as std::ffi::c_int as BYTE;
    DDesc.nbBytes = 0 as std::ffi::c_int as BYTE;
    HUF_fillDTableX6LevelN(
        DDescription,
        DSequence,
        memLog as std::ffi::c_int,
...
unsafe extern "C" fn HUF_decodeSymbolX6(
    ...
    memcpy(
        op,
        ds.offset(val as isize) as *const std::ffi::c_void,
        ::core::mem::size_of::<HUF_DSeqX6>() as std::ffi::c_ulong,
    );
...
unsafe extern "C" fn HUF_decodeStreamX6(
    ...
    let mut ddPtr = DTable.offset(1 as std::ffi::c_int as isize)
    ...
    let mut dsPtr = DTable
        ...
        .offset(
            ((1 as std::ffi::c_int as size_t)
                << dtLog.wrapping_sub(1 as std::ffi::c_int as U32)) as isize,
        ) as *const std::ffi::c_void;
    let mut ds = dsPtr as *const HUF_DSeqX6;
    let pStart = p;
    while BIT_reloadDStream(bitDPtr) as std::ffi::c_uint
        == BIT_DStream_unfinished as std::ffi::c_int as std::ffi::c_uint
        && p <= pEnd.offset(-(16 as std::ffi::c_int as isize))
    {
        if MEM_64bits() != 0 {
            p = p
                .offset(
        ...
        p = p
            .offset(
                HUF_decodeSymbolX6(p as *mut std::ffi::c_void, bitDPtr, dd, ds, dtLog)
                    as isize,
            );
    }
    while BIT_reloadDStream(bitDPtr) as std::ffi::c_uint
    ...
    while p <= pEnd.offset(-(4 as std::ffi::c_int as isize)) {
        p = p
            .offset(
                HUF_decodeSymbolX6(p as *mut std::ffi::c_void, bitDPtr, dd, ds, dtLog)
                    as isize,
    ...
    while p < pEnd {
        p = p
            .offset(
                HUF_decodeLastSymbolsX6(
                    p as *mut std::ffi::c_void,
                    pEnd.offset_from(p) as std::ffi::c_long as U32,
                    bitDPtr,
                    dd,
                    ds,
                    dtLog,
...
unsafe extern "C" fn HUF_decompress4X6_usingDTable(
    ...
    endSignal = BIT_reloadDStream(&mut bitD1) as std::ffi::c_uint
        ...
        | BIT_reloadDStream(&mut bitD4) as std::ffi::c_uint;
    while op3 <= opStart4
        && endSignal == BIT_DStream_unfinished as std::ffi::c_int as U32
        && op4 <= oend.offset(-(16 as std::ffi::c_int as isize))
    {
        if MEM_64bits() != 0 {
            op1 = op1
                .offset(
...
unsafe extern "C" fn HUF_decompress4X6(
    ...
    let mut DTable: [std::ffi::c_uint; 6145] = [
        12 as std::ffi::c_int as std::ffi::c_uint,
        0,
        0,
        0,
...
unsafe extern "C" fn HUF_decompress(
    ...
    if Dtime[2 as std::ffi::c_int as usize] < Dtime[algoNb as usize] {
        algoNb = 2 as std::ffi::c_int as U32;
    }
    ...
    *dumpsLengthPtr = dumpsLength;
    ...
    match Offtype {
        2 => {
            ...
            FSE_buildDTable_rle(
                DTableOffb,
                (*fresh36 as std::ffi::c_int & MaxOff) as std::ffi::c_uchar,
            );
        }
        1 => {
...
#[no_mangle]
pub unsafe extern "C" fn ZSTDv02_isError(mut code: size_t) -> std::ffi::c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn ZSTDv02_freeDCtx(mut dctx: *mut ZSTDv02_Dctx) -> size_t {
    return ZSTD_freeDCtx(dctx as *mut ZSTD_DCtx);
}
#[no_mangle]
pub unsafe extern "C" fn ZSTDv02_resetDCtx(mut dctx: *mut ZSTDv02_Dctx) -> size_t {
