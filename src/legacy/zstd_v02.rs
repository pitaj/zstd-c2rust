...
unsafe extern "C" fn HUF_fillDTableX6LevelN(
    ...
    while s < sortedListSize {
        ...
        let start = rankVal[weight as usize];
        let length = ((1 as libc::c_int) << sizeLog - nbBits) as u32;
        baseSeq.byte[level as usize] = symbol;
        DDesc.nbBits = totalBits as u8;
        if level < 3 as libc::c_int as libc::c_uint && sizeLog - totalBits >= minBits {
            let mut nextMinWeight = totalBits + scaleLog;
            if nextMinWeight < 1 as libc::c_int {
...
unsafe extern "C" fn HUF_decodeSymbolX6(
    ...
    memcpy(
        ...
        ::core::mem::size_of::<HUF_DSeqX6>() as libc::c_ulong,
    );
...
unsafe extern "C" fn HUF_decodeStreamX6(
    ...
    let mut ddPtr = DTable.offset(1 as libc::c_int as isize) as *const libc::c_void;
    ...
    while BIT_reloadDStream(bitDPtr) as libc::c_uint
        == BIT_DStream_unfinished as libc::c_int as libc::c_uint
        && p <= pEnd.offset(-(16 as libc::c_int as isize))
    {
        if MEM_64bits() != 0 {
            p = p
                .offset(
    ...
    while p <= pEnd.offset(-(4 as libc::c_int as isize)) {
        p = p
            .offset(
                HUF_decodeSymbolX6(p as *mut libc::c_void, bitDPtr, dd, ds, dtLog)
                    as isize,
...
unsafe extern "C" fn HUF_decompress4X6_usingDTable(
    ...
    while op3 <= opStart4
        ...
        && op4 <= oend.offset(-(16 as libc::c_int as isize))
    {
        if MEM_64bits() != 0 {
            op1 = op1
                .offset(
