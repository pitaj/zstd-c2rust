...
unsafe extern "C" fn HUF_decodeSymbolX2(
    ...
    libc::memcpy(
        ...
        ZSTD_memcpy!(op, & dt[val].sequence, 2),
        ZSTD_memcpy!(op, & dt[val].sequence, 2) as libc::size_t,
    );
    BIT_skipBits(DStream, (*dt.offset(val as isize)).nbBits as U32);
...
unsafe extern "C" fn HUF_decodeLastSymbolX2(
    ...
    libc::memcpy(
        ...
        ZSTD_memcpy!(op, & dt[val].sequence, 1),
        ZSTD_memcpy!(op, & dt[val].sequence, 1) as libc::size_t,
    );
...
pub unsafe extern "C" fn HUF_decompress1X_DCtx_wksp(
    ...
    if cSrcSize == dstSize {
        libc::memcpy(
            ...
            ZSTD_memcpy!(dst, cSrc, dstSize),
            ZSTD_memcpy!(dst, cSrc, dstSize) as libc::size_t,
        );
        return dstSize;
    }
    if cSrcSize == 1 as std::ffi::c_int as size_t {
        libc::memset(
            ...
            ZSTD_memset!(dst, * (const BYTE *) cSrc, dstSize),
            ZSTD_memset!(dst, * (const BYTE *) cSrc, dstSize) as libc::size_t,
        );
        return dstSize;
    }
